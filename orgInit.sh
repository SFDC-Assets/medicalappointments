...
cat hc.out | awk 'FNR==2{ print substr($4,1,length($4)-1) }' > EVL_EMAIL.PROPERTY.name 
cat hc.out | awk 'FNR==2{ print substr($5,1,length($5)-1) }' > EVL_PEM_PROPERTY.name
heroku config:get $(cat EVL_EMAIL.PROPERTY.name) -a einstein-vl-emea > EVL_EMAIL.PROPERTY.value
...
sfdx shane:ai:playground:setup -f einstein_platform.pem -e $(cat EVL_EMAIL.PROPERTY.value) -k kOgFOPDWWfCFIOhtyobwXw==
